...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, include = T)
```
...
### Change in share of employment in Advanced Industries--LOU and peers
...
pct_ai <- function(df)df%>%
  ...
  left_join(metro.data::ai, by = c("naics" = "code_naics4_2017")) %>%
  left_join(county_cbsa_st[c("stco_code", "cbsa_code", "cbsa_name")]) %>%
...
### R&D per 1,000 workers--LOU and peers
...
univ_rd_18 %>%
  ...
  filter(univ_name == "University of Louisville")%>%
...
### Startups launched at universities--LOU and peers
...
### INC 5000 high growth companies--LOU and peers
...
### Startups per 1,000 workers--LOU and peers
```{r startup}
...
## Skills
### % of population 25+ with AA/BA--LOU and peers
```{r}
cbsa_output %>%
...
### Employment-to-population ratio 18-64--LOU and peers

```{r epratio, echo=F, include=T}
...
### Prevalence of health problems--LOU and peers
```{r}
library(RSocrata)
...
### Struggling families? (I think this may not be available?)--LOU and peers

not available

### Share of workers who are low-wage--LOU and peers
...
### Share of population out-of-work--LOU and peers
only available for core counties
```{r oow}
...
co_oow %>%
  ...
  select(-contains("name"), -contains("population")) %>%
  pivot_wider(names_from = "age", values_from = "pct_oow")%>%
  left_join(county_cbsa_st[c("stco_code", "stco_name", "cbsa_name")], by = "stco_code")%>%
  knitr::kable()
```


### Digital skills demand, 2002/2016--LOU
...
## Access
### Job density change by block group?--LOU
we don't have block group data in the warehouse yet - can ask Joanne

### Geographic of prosperity trend (UVA data)--LOU + MW/Southeastern trendlines

http://statchatva.org/changing-shape-of-american-cities/

### Prevalence of health problems by census tract?--LOU


```{r map, include=FALSE}
library(sf)
map <- tidycensus::get_acs(geography = "tract", variables = "B01003_001", state = c("KY"), geometry = T)
map2 <- tidycensus::get_acs(geography = "tract", variables = "B01003_001", state = c("IN"), geometry = T)
...
### Number of jobs reachable by travel time (including public transit)--LOU
